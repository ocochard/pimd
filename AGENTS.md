...
`shared-lan`, `shared-lan-spt`, `ssm`, `ssm-range`, `alias`, `ifgone`); see the script header for the
...
on a subnet the VIF does not own, and `ifgone` the only one about what pimd does when an interface it
has a VIF on is destroyed underneath it.
