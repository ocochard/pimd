...
** Update the ChangeLog and bump the version in configure.ac

The release pass over the [UNRELEASED] section of ChangeLog.md, and
AC_INIT, which still says 3.0-beta1.  Last thing before the tag, listed so
it is not what everybody assumes somebody else did.


* pimd 3.1 release:
** Drop privileges after init, upstream issue #115

An option, -p USER[:GROUP], to leave the daemon running as somebody other
than root once it is up.  It cannot be a setuid() at the end of main():
restart() (src/main.c) closes and reopens the IGMP and PIM raw sockets, the
routing socket and the kernel's MFC on every SIGHUP and every `pimctl
restart`, and touches the PID file, all of which want the privileges that
were just given away.  So this wants the split the upstream issue names, a
privileged parent holding the sockets and a lesser child inheriting them,
which is a structural change and not a thing to land beside a release.
...
** Use the yacc config parser from mrouted
...
Worth deciding whether it survives at all: Quagga is unmaintained, so the
question today is an FRR interface or none.


* Random issues:
** Run the code checker sparse harder on the code: CC=cgcc make

** Work through doc/rfc7761-compliance.md

The list itself is done: every normative section of the RFC has been read
against the code and what disagrees is written up there, with a file:line, an
effort estimate and a `Test:` note per entry.  The input validation and trust
entries are listed first and all six are fixed, and no parser entry is left.

What is left open is four entries.  M7, a traffic-driven Keepalive Timer, and
S1, SSM groups that carry no RP, are state pimd does not keep, so each is a
structural change rather than a check.  A3 and A4 stay open on purpose, one
because the kernel decapsulates a Register before the daemon is handed
anything, the other because sec. 6.4 describes rather than prescribes.  None
of the four is worth holding a release for.

What the list has never covered is RFC 5059, which owns the BSR mechanism
sec. 4.7 names without specifying, and RFC 5796, which sec. 6.3 defers IPsec
to and which asks nothing of this daemon; see the entry below.
...
** pimd still has to find two enabled phyints at the moment it starts

Done: interfaces are rescanned at runtime now, so one configured after the
daemon is up becomes a vif of its own -- rescan_vifs() (src/vif.c), off the
notifications routesock.c and netlink.c read from the event loop, with a
periodic scan under them.  The ifnew scenario of test/lab.sh covers it.

What is left is the other half of the same rc(8) ordering.  init_vifs()
ends in "Cannot forward: no enabled vifs" unless at least one phyint is
enabled when it runs, and that is exit(-1): a router whose only links are
negotiated -- PPP, L2TP, a tunnel that comes up -- still has to be started
with -w SEC to wait for them, or -r to retry forever, or after the thing
that builds them.  Waiting for the first vif to appear instead, and
starting to forward when it does, would make both knobs unnecessary; it
needs a daemon that can sit with an empty vif table without the rest of the
code assuming phys_vif is set.
