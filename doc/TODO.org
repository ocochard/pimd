...
Open.  The prefix matcher does not cover these: string_match()
(src/pimctl.c) compares the shorter of the two words, so "show if" is
tested against "show igmp" as "if" vs "ig" and against "show interface" as
"if" vs "in", and matches neither.  "show routes" and "show groups" have no
command to abbreviate at all, the commands are "show mrt" and "show igmp".
Next after "show mfc": same file, no protocol code, and it is what users
type first.
