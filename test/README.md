...
| `igmpv3.c`  | Sends exactly one IGMP message and exits: a v3 membership report by default, a v2 or v1 one with `-v`, a v2 leave with `-L`, or a general query of any of the three versions with `-Q`.  Needed wherever a test has to watch a membership *age out*: a kernel that joined a group answers every query afterwards, so the membership never expires while the emulated device is on the LAN.  The older versions and the query are what `igmp-compat` drives -- no host sends a v1 report any more, and a query is a router's message, so neither can be had from a kernel. |
